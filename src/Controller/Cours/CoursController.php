...
class CoursController extends AbstractController
{
     ...
     * @throws Exception
     */
    public function ajax(Request $request, CheckCoursRepo $checkCoursRepo, CheckSession $checkSession): Response
    {
        if ($request->isXmlHttpRequest()) {
            ...
            if (is_numeric($request->query->get('date'))) {
                ...
                throw new Exception("Problème pour la récupération du jour. Vérifiez si javascript est activé.");
            }
            ...
            if ($role === "ROLE_ELEVE") {
                ...
                if ($select[0] === 'pr') {
                    if ($role === "ROLE_PROF") {
                        $profId = $checkSession->getSession($request)->get('user')->getId();
                    } else {
                        $profId = $select[1];
                    }
    ...
    public function index(Request $request, CheckSession $checkSession): Response
    {
        ...
        $listes = $this->getLists($role, $session);

        return $this->render('cours/index.html.twig', [
            'current_menu' => 'cours',
            'start_hour' => $this->getParameter('startTimeTable'),
            'liste_classe' => $listes['liste_classe'],
            'liste_sousgroupe' => $listes['liste_sousgroupe'],
            'liste_prof' => $listes['liste_prof'],
        ]);
    }
    ...
    private function getLists($role, $session)
    {
